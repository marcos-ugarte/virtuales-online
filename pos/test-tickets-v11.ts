/**
 * Test script v11: Session recovery + Game rotation on locked rounds
 *
 * Improvements over v10:
 * 1. Properly detect and recover from session expiration
 * 2. When a round is locked, try a different game instead of retrying same game
 * 3. Track session state through console messages
 */
...
// Track if session needs to be re-established
...
async function forceLogin(page: Page): Promise<boolean> {
  try {
    console.log('   [Login] Full re-login...')
...
async function ensureLoggedIn(page: Page): Promise<boolean> {
  // If we know session is invalid, force re-login
  if (sessionNeedsRelogin) {
    console.log('   [Session] Needs re-login due to prior auth failure')
...
async function createTicket(
  ...
  // If a game is locked, try the next game in rotation
  const gameIndex = (attempt - 1 + retryCount) % games.length
  const game = gameOverride || games[gameIndex]
  ...
  try {
    ...
    if (pendingTicketResult.resolved) {
      if (pendingTicketResult.success && isValidTicketId(pendingTicketResult.ticketId)) {
        ...
        // Handle "Not authenticated" - force re-login and retry
        if (result.error.includes('Not authenticated') && retryCount < CONFIG.MAX_RETRIES) {
          console.log('   [Retry] Session lost, forcing re-login...')
          sessionNeedsRelogin = true
          await delay(2000)
          return createTicket(page, attempt, retryCount + 1)
        }

        // Handle "not open" - try a DIFFERENT game
        if (result.error.includes('not open') && retryCount < CONFIG.MAX_RETRIES) {
          const nextGame = games[(gameIndex + 1) % games.length]
          console.log(`   [Retry] Round locked, trying ${nextGame.toUpperCase()} instead...`)
          await delay(2000)
          return createTicket(page, attempt, retryCount + 1, nextGame)
        }
      } else {
      ...
      if (retryCount < CONFIG.MAX_RETRIES) {
        console.log('   [Retry] No response, retrying...')
        await delay(3000)
        return createTicket(page, attempt, retryCount + 1)
      }
    }
...
function printSummary(): void {
  ...
  fs.writeFileSync('test-results/results-v11.json', JSON.stringify(results, null, 2))
  console.log('\nResults saved to test-results/results-v11.json')
}
...
async function main() {
  ...
  console.log('='.repeat(60))
  console.log('TICKET TEST v11 - Session Recovery + Game Rotation')
  console.log('='.repeat(60))
  ...
  page.on('console', msg => {
    ...
    if (text.includes('User not authenticated') || text.includes('not authenticated')) {
      ...
      sessionNeedsRelogin = true
      console.log('   [API] Session lost - will re-login')
    }
  })
