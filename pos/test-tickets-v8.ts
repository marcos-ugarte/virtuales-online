/**
 * Test script v8: Optimized timing for 100% success
 *
 * Key improvements:
 * - Higher MIN_COUNTDOWN (50s) to ensure enough time
 * - Countdown check after EACH step
 * - Faster UI interactions (reduced delays)
 * - Smart wait for next round if countdown too low
 * - Skip DOT game (has different UI for SUMA input)
 */
...
const CONFIG = {
  ...
  MIN_COUNTDOWN: 50,  // Need at least 50s to complete all steps
  ABORT_COUNTDOWN: 15, // Abort if countdown drops below this during process
  MAX_RETRIES: 2,
}
...
// Skip DOT - use only DOS, DOE, HOC (they have same UI)
const games = ['dos', 'doe', 'hoc']
...
async function forceClick(page: Page, locator: any, name: string): Promise<boolean> {
  try {
    // Try force click directly - faster than waiting
...
async function waitForSafeCountdown(page: Page): Promise<number> {
  ...
  return 0 // Failed to get safe countdown
}
...
async function createTicket(page: Page, attempt: number, retryCount: number = 0): Promise<TicketResult> {
  ...
  try {
    ...
    if (currentCountdown < CONFIG.ABORT_COUNTDOWN) {
      console.log(`   [Abort] Countdown dropped to ${currentCountdown}s`)
      result.error = 'Countdown dropped'
      result.duration = Date.now() - startTime
      if (retryCount < CONFIG.MAX_RETRIES) {
    ...
    // 6. Select runner (force click - faster)
    ...
    // 7. Check countdown after runner
    ...
    // 9. Final countdown check before submit
    currentCountdown = await getCountdown(page)
    if (currentCountdown < CONFIG.ABORT_COUNTDOWN) {
      console.log(`   [Abort] Countdown critical (${currentCountdown}s)`)
    ...
    if (pendingTicketResult.resolved) {
      if (pendingTicketResult.success && isValidTicketId(pendingTicketResult.ticketId)) {
        ...
        // Retry on "GameRound not open" - timing issue
...
function printSummary(): void {
  ...
  fs.writeFileSync('test-results/results-v8.json', JSON.stringify(results, null, 2))
  console.log('\nResults saved to test-results/results-v8.json')
}
...
async function main() {
  ...
  console.log('='.repeat(60))
  console.log('TICKET TEST v8 - Optimized Timing')
  console.log('='.repeat(60))
