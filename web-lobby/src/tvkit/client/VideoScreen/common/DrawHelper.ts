/**
 * VENDORED DrawHelper (canvas texture helper) — for the WinnerDog port.
 *
 * Copied from streaming_kit client/src/VideoScreen/common/DrawHelper.ts, with a
 * SINGLE trim: the only method that imported `Engine`
 * (`createCustomSkewedRoundedRectangleTexture`, which calls
 * `Engine.instance.getPixiApp().renderer.generateTexture(...)`) is removed,
 * because WinnerDog / WinnerDogLine never call it and `Engine` is not part of
 * the vendored graph. Everything WinnerDog actually uses
 * (`createSkewedRoundedRectangleTexture`, `drawPatternTexture`) is verbatim.
 * Util.rgbToHex (used by drawPatternTexture) is provided by the trimmed
 * common/Util. The module-hot dispose hook is kept verbatim.
 */
...
// streaming_kit used webpack's `module.hot` to dispose cached textures on HMR.
// Under Vite/ESM there is no `module` global, so guard the reference (it would
// otherwise throw a ReferenceError at module load). Vite HMR uses import.meta.hot.
const hm: any = typeof module !== "undefined" ? (module as any) : (import.meta as any).hot ? { hot: (import.meta as any).hot } : undefined;
if (hm && hm.hot) {
