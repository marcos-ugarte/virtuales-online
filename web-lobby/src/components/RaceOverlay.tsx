/**
 * RaceOverlay — transparent PIXI canvas drawn ON TOP of the race <video> in the
 * lobby's LiveMonitor. Ported from tvbox-online (which proved the pipeline); the
 * vendored streaming_kit overlay components live UNMODIFIED under src/tvkit/.
 *
 * It ports the REAL streaming_kit RaceBarDog (top-right race number + in-race
 * timer), RaceIntervalsDog (mid-race top-2 leaders box) and WinnerDog ×3 (final
 * winners panel). They are driven by a SHIM `Logic` (src/tvkit/client/Logic/
 * Logic.ts): because our race videos are race-only (no intro), Logic collapses
 * to constants and time comes straight from the live `<video>.currentTime`.
 *
 * Unlike the tvbox-online original (which hardcoded 'dog6'), this version takes
 * the REAL gameType ('dog6' | 'dog8') so colours / runner-count / interval
 * durations are correct for both dog games. Design space is 1280x720; the canvas
 * is CSS-stretched to fill the `.lm-video` box.
 */
...
export type OverlayGameType = 'dog6' | 'dog8';
...
export interface RaceOverlayProps {
  /** Real dog game type — drives runner count / colours / interval durations. */
  gameType: OverlayGameType;
  raceId: string;
...
export function RaceOverlay(props: RaceOverlayProps) {
  ...
  // The overlay is rebuilt when the gameType changes (dog6 ↔ dog8) because the
  // ported components bake the type into their constructor (runner count /
  // colours). Data updates within a type ride through propsRef in the tick.
  ...
  useEffect(() => {
    ...
    fontsReady.then(() => {
      ...
      // oddsAlwaysOn=true (our skin is the always-on skin, 11).
      const raceBar = new RaceBarDog(gameType, 240, Logic.languageId, false, true);
      raceBar.width = 185;
      raceBar.height = 61;
      ...
      const raceIntervals = new RaceIntervalsDog(gameType);
      ...
      // forecast reveal @40.5s and run for the rest of the race.
      ...
      for (let i = 0; i < 3; i++) {
        const w = new WinnerDog(gameType, 240, winnerAnims[i], winnerTypes[i], Logic.languageId);
      ...
      const tick = () => {
        ...
        // by fillRace(result.clockEndTime).
        ...
        if (sig !== roundSigRef.current) {
          ...
          raceIntervals.fill(EMPTY_TRACK, toRaceIntervals(gameType, p.interval), drivers);
        ...
        // panel lingered into the countdown to the next race.
    ...
    // Rebuild when the dog game type changes — the ported components bake it in.
  }, [gameType]);
