...
/**
 * If we were already this far into the race (or more) when the clip first
 * mounted, treat it as a genuine mid-race join and seek to the live elapsed.
 * Below this we play from frame 0 so the salida (the start of the race) is
 * always visible — the common case where we were watching when it started
 * but the mp4 simply took a couple of seconds to load.
 */
const LATE_JOIN_THRESHOLD_SEC = 10;

/**
 * VideoPlayer — plays the recorded race mp4. When we caught the race at (or
 * near) its start we play from the beginning so the salida is shown; only a
 * clearly-late viewer is seeked to the live elapsed since videoStartDt.
 * Re-evaluated whenever the URL changes (i.e. the picked race advances).
 */
  ...
  return (
    <aside className="live-monitor" aria-label="Live race monitor">
      ...
      <div className="lm-video">
        ...
        {/* PIXI overlay — race number / countdown / winners, drawn on top of
            whatever the video box currently shows. */}
        {pickedRace && pickedTimer && (
          <RaceOverlay
